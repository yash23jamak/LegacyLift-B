...
/**
 * Project Migration prompt
 * latest migration prompt 08-12-2025 (MINIMAL: components + pages only)
 */
// export const MIGRATION_PROMPT = `
// You are converting a legacy JSP (Java Server Pages) web application into a fully client-side, modern React project.
// Generate a clean, production-ready Vite + TypeScript React codebase using officially supported, stable frontend libraries.
// All configuration and source files must be syntactically valid, compilable, runnable, and properly formatted.
...
// - Icons: @heroicons/react
// - Authentication-aware: Use .env variables for API URLs or tokens
// - Accessibility: Semantic HTML with ARIA roles and keyboard navigation
// - Explicitly exclude mobile/native dependencies, Expo modules, unit testing tools, linting/formatting configs, Redux/React Query stores, and Axios service files.

// ## 2. Migration Behavior and Goals
// - Convert JSP templates and includes into modular, functional React components.
// - Convert JSP pages and server-side routing into React Router pages.
// - Keep component naming consistent with legacy features.
// - Provide one simple JSP-to-React example page with inline data fetching (optional Axios inside the page) and routing.
// - Automatically remove unused or duplicated files.

// ## 3. Output File and Project Structure
// Provide only the minimal required files and folders:
// - package.json
// - vite.config.ts
// - tsconfig.json
// - postcss.config.cjs
...
// - src/store/
// - src/services/
// - Vitest, ESLint, Prettier, and related configuration files
// - Additional documentation files

// ## 4. Dependency and Version Integrity
// Install and validate only required dependencies:
// - react
// - react-dom
// - vite
// - @vitejs/plugin-react
...
// Checks:
// 1. All peer dependencies satisfied.
// 2. No deprecated flags or unresolved plugins.
// 3. Project compiles and runs on Node 18+.
// If conflicts or missing versions occur, automatically upgrade to compatible versions.

// ## 5. Configuration Standards
// - .ts and .js files must use ESM imports/exports.
// - .cjs files must use CommonJS (module.exports).
// - No escaped newlines or quotes; produce real files.
// - HTML and TSX must be valid and uncorrupted.

// ## 6. Minimal Validation
// Before final output:
// 1. Simulate install and build; ensure no peer conflicts.
// 2. Run \`vite dev\` and confirm Tailwind renders.
// 3. Confirm routing works (Home and Users pages).
// 4. Verify all import paths and case sensitivity.
// 5. Ensure no corrupt JSX, escaped sequences, or unused code.
...
//   { "name": "vite.config.ts", "content": "import { defineConfig } from 'vite'..." }
// ]
// Each "content" must be real code. All files must run as-is when written to disk.
...
// - Tailwind working
// - Routing functional
// - No tests, no ESLint/Prettier, no store/services
// - Correct ES/CJS syntax
